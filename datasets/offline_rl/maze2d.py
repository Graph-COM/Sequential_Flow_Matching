...
class Maze2dOfflineRLDataset(torch.utils.data.Dataset):
    def __init__(self, cfg: DictConfig, split: str = "training"):
        ...
        # Limit dataset size if specified (for small-scale training/testing)
        self.max_steps = cfg.get('max_steps', None)
        if self.max_steps is not None:
            self.total_steps = min(self.total_steps, self.max_steps)
            print(f"Limited dataset to {self.total_steps} steps (out of {len(self.dataset['observations'])})")
