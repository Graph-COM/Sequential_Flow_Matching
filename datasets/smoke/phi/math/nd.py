...
def _forward_divergence_nd(field):
    ...
    for dimension in dims:
        ...
        diff = vq[tuple([slice(None)]+upper_slices)] - vq[tuple([slice(None)]+lower_slices)]
...
def _central_divergence_nd(tensor):
    ...
    for dimension in dims:
        ...
        diff = tensor[tuple([slice(None)] + upper_slices + [rank - dimension - 1])] - \
               tensor[tuple([slice(None)] + lower_slices + [rank - dimension - 1])]
...
def _central_diff_nd(field, dims):
    ...
    for dimension in dims:
        upper_slices = [(slice(2, None) if i==dimension else slice(1,-1)) for i in dims]
        lower_slices = [(slice(-2)      if i==dimension else slice(1,-1)) for i in dims]
        diff = field[tuple([slice(None)] + upper_slices + [0])] - field[tuple([slice(None)] + lower_slices + [0])]
        df_dq.append(diff)
    return math.stack(df_dq[::-1], axis=-1)
...
def _sliced_laplace_nd(tensor):
    ...
    for dimension in dims:
        ...
        upper_slices = [(slice(2, None) if i == dimension else slice(1,-1)) for i in dims]
        lower_slices = [(slice(-2) if i == dimension else slice(1,-1)) for i in dims]
        diff = tensor[tuple([slice(None)] + upper_slices + [slice(None)])] \
               + tensor[tuple([slice(None)] + lower_slices + [slice(None)])] \
               - 2 * tensor[tuple([slice(None)] + center_slices + [slice(None)])]
        components.append(diff)
...
def _weighted_sliced_laplace_nd(tensor, weights):
    ...
    for dimension in dims:
        ...
        lower_weights = weights[tuple([slice(None)] + lower_slices + [slice(None)])] * weights[tuple([slice(None)] + center_slices + [slice(None)])]
        upper_weights = weights[tuple([slice(None)] + upper_slices + [slice(None)])] * weights[tuple([slice(None)] + center_slices + [slice(None)])]
        center_weights = - lower_weights - upper_weights

        lower_values = tensor[tuple([slice(None)] + lower_slices + [slice(None)])]
        upper_values = tensor[tuple([slice(None)] + upper_slices + [slice(None)])]
        center_values = tensor[tuple([slice(None)] + center_slices + [slice(None)])]
...
def downsample2x(tensor, interpolation="LINEAR"):
    ...
    for dimension in dims:
        ...
        sum = tensor[tuple([slice(None)]+upper_slices+[slice(None)])] + tensor[tuple([slice(None)]+lower_slices+[slice(None)])]
        tensor = sum / 2
    return tensor
...
def upsample2x(tensor, interpolation="LINEAR"):
    ...
    for dim in dims:
        ...
        left = 0.75 * tensor[tuple([slice(None)]+left_slices_2+[slice(None)])] + 0.25 * tensor[tuple([slice(None)]+left_slices_1+[slice(None)])]
        right = 0.25 * tensor[tuple([slice(None)]+right_slices_2+[slice(None)])] + 0.75 * tensor[tuple([slice(None)]+right_slices_1+[slice(None)])]
...
class StaggeredGrid:
        ...
        for d in dims:  # z,y,x
            if d == face_dimension_zyx:
                ...
                for d2 in dims:  # z,y,x
                    ...
                    t = t[tuple([slice(None)] + slices1)] + t[tuple([slice(None)] + slices2)]
                    if d2 == d:
    ...
    def from_scalar(scalar_field, axis_forces, padding_mode="constant"):
        ...
        for dimension in dims: # z,y,x
            ...
            upper_slices = [(slice(1, None) if i == dimension else slice(None)) for i in dims]
            lower_slices = [(slice(-1) if i == dimension else slice(None)) for i in dims]
            neighbour_sum = padded_field[tuple([slice(None)] + upper_slices + [slice(None)])] + \
                            padded_field[tuple([slice(None)] + lower_slices + [slice(None)])]
