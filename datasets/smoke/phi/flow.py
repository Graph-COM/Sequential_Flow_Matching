    ...
    def set_obstacle(self, mask_or_size, origin=None):
        ...
        if isinstance(mask_or_size, np.ndarray):
            ...
            if origin is None:
                ...
                origin = list(origin)
            self._fluid_mask[tuple([0] + [slice(origin[i], origin[i] + mask_or_size[i]) for i in dims] + [0])] = 0
            self._active_mask[tuple([0] + [slice(origin[i], origin[i] + mask_or_size[i]) for i in dims] + [0])] = 0
        self._velocity_mask = self._boundary.create_velocity_mask(self._fluid_mask, self._dimensions, self._mac)
    ...
    def _create_centered_velocity_mask(self, fluid_mask, spatial_dimensions=None):
        ...
        for d in dims:
            ...
            bc_d = math.minimum(math.minimum(
                                fluid_mask[tuple([slice(None)] + upper_slices + [slice(None)])],
                                fluid_mask[tuple([slice(None)] + center_slices + [slice(None)])]),
                                fluid_mask[tuple([slice(None)] + lower_slices + [slice(None)])])
            bcs.append(bc_d)
