...
def log_stream_video(xs, xs_pred, logger, fvd_model):
    # for now, just log the first video
    xs = xs[:, :, 0]
    xs_pred = xs_pred[:, :, 0]
...
def get_validation_metrics_for_stream_videos(
    observation_hat,
    observation_gt,
    lpips_model: Optional[LearnedPerceptualImagePatchSimilarity] = None,
    fid_model: Optional[FrechetInceptionDistance] = None,
    ...
    :param fid_model: a FrechetInceptionDistance object  from algorithm.common.metrics
    :param fvd_model: a FrechetVideoDistance object  from algorithm.common.metrics
    :param num_gen_trials: number of generative trials
    :return: a tuple of metrics
    """
    #if observation_hat.ndim == 6:
    windows, frame, batch, channel, height, width = observation_hat.shape
    if num_gen_trials is not None and num_gen_trials > 1:
        assert batch % num_gen_trials == 0
        real_batch = batch // num_gen_trials
        #diffusion_trials = 1
    #elif observation_hat.ndim == 7:
        #frame, batch, diffusion_trials, windows, channel, height, width = observation_hat.shape
    #else:
        #raise Exception("Invalid format of streaming video prediction evaluation.")
    output_dict = {}
    observation_gt = observation_gt.type_as(observation_hat)  # some metrics don't fully support fp16
    ...
    #observation_hat = observation_hat.view(-1, channel, height, width)
    #observation_gt = observation_gt.view(-1, channel, height, width)
    # reshape to (frame, batch * diffusion_trials, number_windows, channel, height, width)
    #if diffusion_trials > 1:
        #observation_hat = observation_hat.flatten(1, 2)
        #observation_gt = observation_gt.flatten(1, 2)

    # error at each prediction window
    rmse = [[] for _ in range(windows)]
    psnr = [[] for _ in range(windows)]
    ssim = [[] for _ in range(windows)]
    uiqi = [[] for _ in range(windows)]
    fvd = []
    lpips_list = [[] for _ in range(windows)]
    energy_score_list = []
    import lpips
    validation_lpips_model = lpips.LPIPS(net='vgg').to(observation_hat.device)
    for window_id in range(windows):
        if fvd_model is not None:
            #output_dict["fvd"] = fvd_model.compute(
                #torch.clamp(observation_hat[window_id], -1.0, 1.0),
                #torch.clamp(observation_gt[window_id], -1.0, 1.0),
            #)
            fvd.append(fvd_model.compute(
                torch.clamp(observation_hat[window_id], -1.0, 1.0),
                torch.clamp(observation_gt[window_id], -1.0, 1.0),
            ))
        if num_gen_trials is not None and num_gen_trials > 1:
            xhat = observation_hat[window_id]
            xhat = xhat.view([frame, real_batch, num_gen_trials, channel, height, width])
            x = observation_gt[window_id]
            x = x.view([frame, real_batch, num_gen_trials, channel, height, width])
            x = x[:, :, 0] # x are repeated in num_gen_trials, so just take one copy
            energy_score_list.append(energy_score(xhat, x))

        for lead_time in range(frame):
            print('calculating metric for %d-time-%d-frame' % (window_id, lead_time))
            #pred_time = start_time + lead_time
            #output_key_suffix = '_start_%d_pred_%d' % (start_time, pred_time)
            xhat = observation_hat[window_id, lead_time]
            x = observation_gt[window_id, lead_time]
            rmse[window_id].append(mean_squared_error(xhat, x).sqrt())
            psnr[window_id].append(peak_signal_noise_ratio(xhat, x, data_range=2.0))
            ssim[window_id].append(structural_similarity_index_measure(xhat, x, data_range=2.0))
            uiqi[window_id].append(universal_image_quality_index(xhat, x))
            lpips_list[window_id].append(validation_lpips_model(torch.clamp(xhat, -1.0, 1.0),
                                                                torch.clamp(x, -1.0, 1.0)).mean().item())
            #output_dict["mse"+output_key_suffix] = mean_squared_error(xhat, x)
            #output_dict["mse_std"+output_key_suffix] = (xhat - x).square().std()
            #output_dict["psnr"+output_key_suffix] = peak_signal_noise_ratio(xhat, x, data_range=2.0)
            #output_dict["ssim"+output_key_suffix] = structural_similarity_index_measure(xhat, x, data_range=2.0)
            #output_dict["uiqi"+output_key_suffix] = universal_image_quality_index(xhat, x)

    #output_dict["mse"] = (observation_hat - observation_gt).square().mean()
    #output_dict["mse_std"] = (observation_hat - observation_gt).square().std()
    rmse = np.array(rmse)
    psnr = np.array(psnr)
    ssim = np.array(ssim)
    uiqi = np.array(uiqi)
    lpips_list = np.array(lpips_list)
    energy_scores = np.array(energy_score_list)
    fvd = np.array(fvd)
    output_dict["rmse_per_window"] = rmse.mean(axis=1)
    output_dict["rmse_per_horizon"] = rmse.mean(axis=0)
    output_dict["rmse"] = rmse.mean()
    output_dict["psnr_per_window"] = psnr.mean(axis=1)
    output_dict["psnr_per_horizon"] = psnr.mean(axis=0)
    output_dict["psnr"] = psnr.mean()
    output_dict["ssim_per_window"] = ssim.mean(axis=1)
    output_dict["ssim_per_horizon"] = ssim.mean(axis=0)
    output_dict["ssim"] = ssim.mean()
    output_dict["uiqi_per_window"] = uiqi.mean(axis=1)
    output_dict['uiqi_per_horizon'] = uiqi.mean(axis=0)
    output_dict["uiqi"] = uiqi.mean()
    output_dict['lpips_per_window'] = lpips_list.mean(axis=1)
    output_dict['lpips_per_horizon'] = lpips_list.mean(axis=0)
    output_dict['lpips'] = lpips_list.mean()
    output_dict['energy_score_per_window'] = energy_scores
    output_dict['energy_score'] = energy_scores.mean()
    if fvd_model is not None:
        output_dict['fvd_per_window'] = fvd
        output_dict['fvd'] = fvd.mean()
...
def energy_score(x_pred, x_true, p=1):
    """
    Compute Energy Score for batched video predictions.

    Args:
        x_pred: [T, B, N, *] tensor of model samples
        x_true: [T, B, *] tensor of ground truth
        p: norm power (p=1 recommended; 0<p<2 is proper)
    Returns:
        scalar tensor: mean Energy Score over batch
    """
    x_pred = x_pred.transpose(0, 1).transpose(1, 2) # [B, N, T, *]
    x_true = x_true.transpose(0, 1) # [B, T, *]

    B, N = x_pred.shape[:2]
    x_true = x_true.unsqueeze(1)  # [B, 1, T, *]
    x_flat = x_pred.flatten(2)    # [B, N, D] where D = sum(*)

    # pairwise distances between predicted samples
    diff = x_flat.unsqueeze(2) - x_flat.unsqueeze(1)  # [B, N, N, D]
    d_ij = diff.norm(p=p, dim=-1)  # Lp norm across pixels

    mask = ~torch.eye(N, dtype=bool, device=x_pred.device)
    term1 = d_ij[:, mask].view(B, -1).mean(dim=1)
    #term1 = d_ij.mean(dim=(1, 2))  # E|X-X'|

    # distances between samples and truth
    diff_true = x_flat - x_true.flatten(2)  # [B, N, D]
    d_i = diff_true.norm(p=p, dim=-1)       # |X - x_true|
    term2 = d_i.mean(dim=1)                 # E|X-x|

    es = (term2 - 0.5 * term1) / (x_flat.size(-1))**(1/p)
    return es.mean()  # average over batch