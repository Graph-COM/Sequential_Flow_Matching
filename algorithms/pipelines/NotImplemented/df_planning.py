...
class DiffusionForcingPlanning(DiffusionForcingBase):
    def __init__(self, cfg: DictConfig):
        ...
        self.diffusion_edit_level = cfg.diffusion_edit_level
    ...
    def plan(self, start: torch.Tensor, goal: torch.Tensor, horizon: int, conditions: Optional[Any] = None):
        ...
        for m in range(scheduling_matrix.shape[0] - 1):
            ...
            #(n_tokens, batch_size, *x_stacked_shape)
    ...
    def plan_editing(self, start: torch.Tensor, goal: torch.Tensor, horizon: int, conditions: Optional[Any] = None,
                     old_plan: Optional[torch.Tensor] = None) -> torch.Tensor:
        # start and goal are numpy arrays of shape (b, obs_dim)
        # start and goal are assumed to be normalized
        # returns plan history of (m, t, b, c), where the last dim of m is the fully diffused plan
        ...
        # use old plan as the initial state for diffusion
        #old_noise_level = [0 for _ in range(plan_tokens)]
        # TODO: fix incorrect pyramid level
        start_pyramid_level = self.diffusion_edit_level
        scheduling_matrix = self._generate_scheduling_matrix(plan_tokens)
        chunk = self.diffusion_model.q_sample(old_plan[-1], t=torch.tensor(scheduling_matrix[start_pyramid_level]).to(self.device).unsqueeze(-1).repeat([1, batch_size]))
        scheduling_matrix = scheduling_matrix[start_pyramid_level:]
        #chunk = old_plan[-start_pyramid_level-1]
        #chunk = torch.randn((plan_tokens, batch_size, *self.x_stacked_shape), device=self.device)
        #chunk = self.diffusion_model.q_sample(old_plan, 100)
        #assert old_plan.shape[1] == plan_tokens
        #chunk = old_plan[old_noise_level, torch.arange(plan_tokens)]
        #chunk = torch.clamp(chunk, -self.cfg.diffusion.clip_noise, self.cfg.diffusion.clip_noise)
        pad = torch.zeros((pad_tokens, batch_size, *self.x_stacked_shape), device=self.device)
        init_token = rearrange(self.pad_init(start), "fs b c -> 1 b (fs c)")
        plan = torch.cat([init_token, chunk, pad], 0)
    ...
    def interact(self, batch_size: int, conditions=None, namespace="validation"):
        ...
        while not terminate and steps < self.episode_len:
            ...
            if steps == 0:
                ...
                # plan editing
                plan_hist = self._normalize_x(plan_hist[:, self.open_loop_horizon:]) # drop observed states
                plan_hist = rearrange(plan_hist, "m (t fs) b c -> m t b (fs c)", fs=self.frame_stack)
                plan_hist = self.plan_editing(obs_normalized, goal_normalized, self.episode_len - steps, conditions,
                                              plan_hist)
                # re-planning
                #plan_hist = self.plan(obs_normalized, goal_normalized, self.episode_len - steps, conditions)
