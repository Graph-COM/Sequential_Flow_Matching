...
class Diffusion(nn.Module):
    ...
    def forward_with_given_noise(
        ...
        noised_x = self.q_sample(x_start=x, t=noise_levels, noise=noise)
        model_pred = self.model_predictions(x=noised_x, t=noise_levels, external_cond=external_cond, pad_mask=pad_mask)

        pred = model_pred.model_out
        x_pred = model_pred.pred_x_start

        # apply guidance here if needed
        #if guidance_fn:
            ...
            #)[0]
            #grad = grad.detach() # stop gradient
            #sqrt_one_minus_alpha_t = extract(self.sqrt_one_minus_alphas_cumprod, noise_levels, x.shape)
            #pred_noise = model_pred.pred_noise + sqrt_one_minus_alpha_t * grad
            #pred_x_start = self.predict_start_from_noise(noised_x, noise_levels, pred_noise)
            #x_pred = pred_x_start
            #pred_v = self.predict_v(pred_x_start, noise_levels, pred_noise)
            #if self.objective == "pred_noise":
                #pred = pred_noise
            #elif self.objective == "pred_x0":
                #pred = pred_x_start
    ...
    def sample_step(
        ...
        curr_noise_level = torch.where(curr_noise_level == 0, -1, curr_noise_level)
        next_noise_level = torch.where(next_noise_level == 0, -1, next_noise_level)
    ...
    def ddim_sample_step(
        ...
        if guidance_fn is not None:
            with torch.enable_grad():
                ...
                # TODO: why this is alpha_next in the original code? should be alpha
                #pred_noise = model_pred.pred_noise + (1 - alpha_next).sqrt() * grad
                pred_noise = model_pred.pred_noise + (1 - self.add_shape_channels(alpha)).sqrt() * grad
