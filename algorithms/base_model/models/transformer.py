...
class Transformer(nn.Module):
    def __init__(
        ...
        if second_k_emb:
            self.k_embed_2 = SinusoidalPosEmb(dim=k_embed_dim)
    ...
    def forward(self, x, k, external_cond=None, is_causal=False, t2=None, mask=None):
        # x.shape (T, B, C)
        # k.shape (T, B)
        ...
        if t2 is not None:
            k_embed += rearrange(self.k_embed_2(t2.flatten()), "(t b) d -> t b d", t=seq_len)
        ...
        if is_causal:
            ...
            x = self.transformer(x, src_key_padding_mask=mask, is_causal=is_causal)
        else:
            x = self.transformer(x, mask=None, is_causal=is_causal)
