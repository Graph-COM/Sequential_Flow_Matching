...
class DiffusionForcingBase(BasePytorchAlgo):
    ...
    def forward(self, xs: torch.Tensor, conditions: torch.Tensor, masks: torch.Tensor, context_masks=None) -> torch.Tensor:
        return self.diffusion_model(xs, conditions, noise_levels=self._generate_noise_levels(xs, masks=masks,
    ...
    def _generate_noise_levels(self, xs: torch.Tensor, masks: Optional[torch.Tensor] = None,
        ...
        noise_levels = torch.clamp(noise_levels, 0, 999)
        ...
        if context_masks is not None:
            ...
            noise_levels = torch.where(discard, torch.zeros_like(noise_levels), noise_levels)
    ...
    def sample_t(self, dim, device):
        if self.cfg.noise_level_dist == 'uniform':
            return torch.randint(0, self.timesteps, dim, device=device)
