...
class BasePytorchAlgo(pl.LightningModule, ABC):
    ...
    def forward(self, xs: torch.Tensor, conditions: torch.Tensor, masks: torch.Tensor, context_masks: torch.Tensor=None):
        raise Exception("Please implement forward in the model class")

    @abstractmethod
    def model_sampling(self, xs_context, conditions, prediction_window, n_frames, batch_size, diff_hist=False,
                       guidance_fn=None):
        raise Exception("Please implement model_sampling in the model class")
    ...
    def log_line_plot(
        ...
        **kwargs: Any,
        ...
        # Convert vector to pure Python values to avoid circular references
        if isinstance(vector, torch.Tensor):
            vector = vector.detach().cpu().numpy()
        elif hasattr(vector, 'tolist'):
            vector = vector.tolist()
        elif hasattr(vector, '__iter__') and not isinstance(vector, (str, bytes)):
            vector = [float(v) if hasattr(v, 'item') else v for v in vector]
